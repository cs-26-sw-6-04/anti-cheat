...
| Property | Value |
|----------|-------|
| **Framework** | Google Test (gtest) via CMake CTest |
| **Config file** | `CMakeLists.txt` (tests target) |
| **Quick run command** | `cmake --build --preset debug && sudo ctest --preset debug -R inject\|execve\|proc` |
| **Full suite command** | `cmake --workflow --preset debug && sudo ctest --preset debug` |
...
| 1-01-02 | 01 | 1 | REQ-01 | — | inject enforcer blocks external PROT_EXEC mmap | integration | `sudo ctest --preset debug -R inject` | ❌ W0 | ⬜ pending |
| 1-01-03 | 01 | 1 | REQ-02 | — | execve enforcer blocks unauthorized exec | integration | `sudo ctest --preset debug -R execve` | ❌ W0 | ⬜ pending |
| 1-01-04 | 01 | 1 | REQ-03 | — | proc/openat coverage audit documented and gaps closed | integration | `sudo ctest --preset debug -R proc\|mem` | ⬜ | ⬜ pending |
...
*Existing gtest infrastructure in `tests/` is assumed from STATE.md; Wave 0 adds only new test stubs.*
