...
**Recommendation:** Interpret as (a) — emit deny when `cur_pid()` is in protected subtree AND `prot & PROT_EXEC` AND `file == NULL` (anonymous mapping). This is clear, privacy-preserving, and testable. **Caveat: this rule also blocks JIT-compiled code.** Every modern JIT runtime (V8, Mono/Unity, JVM, LuaJIT, Wine's dynamic loader for Windows DLLs) allocates anonymous PROT_EXEC pages at runtime. For games built with embedded scripting or shipped via Proton, this interpretation blocks the game process itself from JIT-compiling. Interpretation (a) is only viable if the protected target is known to not JIT. Confirm with user before implementing.
